...
namespace MovieServiceDAL.Repositories
{
    public abstract class GenericRepository<T> : IGenericRepository<T> where T : class
    {
        ...
        protected GenericRepository(SqlConnection sqlConnection, IDbTransaction dbTransaction, string tableName, string entityName)
        {
        ...
        {
            string storedProcedure = "SelectAll" + string.Join("", _tableName);

            return await _sqlConnection.QueryAsync<T>($"exec {storedProcedure}",
                transaction: _dbTransaction);
        }

        public async Task<T> GetAsync(int id)
        {
            var result = await _sqlConnection.QuerySingleOrDefaultAsync<T>($"Select{_entityName}ById @Id",
                param: new { Id = id },
                transaction: _dbTransaction);
            if (result == null)
